...
if ($_POST) {
    ...
    try {
        if ($action === 'create') {
            ...
            $userManager->createUser($name, $first_name, $username, $email, $password, $admin);
            showAlert('Utilisateur créé avec succès !', 'success');
            header('Location: users.php');
            exit();
        } elseif ($action === 'edit' && $userId) {
            // Mise à jour d'un utilisateur existant
            $userManager->updateUser($userId, $name, $first_name, $username, $email, $admin);
...
if ($action === 'delete' && $userId && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
...
// Récupérer l'utilisateur pour la modification
$user = null;
        ...
        <div class="flex flex-col md:flex-row">
            ...
            <div class="md:w-64 w-full md:mr-6 mb-6 md:mb-0">
                <div class="sidebar p-6 rounded-xl text-white">
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="admin1.php" class="sidebar-item flex items-center">
                                <i class="fas fa-th-large"></i>
                                <span>Dashboard</span>
                            </a>
                        </li>
                        <li>
                            <a href="users.php" class="sidebar-item active flex items-center">
            ...
            <div class="flex-1">
                ...
                <?php if ($error): ?>
                    <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">
                        <span class="font-medium">Erreur!</span> <?= htmlspecialchars($error) ?>
                    </div>
                <?php endif; ?>
                ...
                <div class="dashboard-header bg-white">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                        <div>
                            ...
                            <p class="text-gray-600">Liste de tous les utilisateurs du site</p>
                        </div>
                        <a href="users.php?action=create" class="btn btn-primary flex items-center mt-4 sm:mt-0">
                ...
                <!-- Statistique -->
                <div class="card bg-white p-6 mb-6 rounded-xl">
                    <div class="flex items-center">
                        <i class="fas fa-users text-2xl text-primary mr-3"></i>
                        <span><?= $userCount ?> utilisateur<?= $userCount > 1 ? 's' : '' ?> au total</span>
                    </div>
                </div>
